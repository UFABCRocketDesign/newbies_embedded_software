...
// Creates a filter and initialize variables
filter *createFilter()
{
...
float mediaMovel(int *sum, filter *values, int newValue, int c)
{
	// "Disregards" the first NUMEROS_MEDIA numbers
	// Inutil
	if (c < NUMEROS_MEDIA)
	{
		*sum += newValue;
	...
	values->front++;
	values->front %= NUMEROS_MEDIA;

	// Decreases the sum value by the number being removed
	*sum -= values->v[values->front];

	// Changes it's value
	values->v[values->front] = newValue;

	// Increases the sum value by the number inserted
	*sum += newValue;

	// Calculates de average and returns
	return (*sum) / NUMEROS_MEDIA;
}

// (~Heitor approved~)™ Official SealⓇ
...
if (h1 >= h2 or h2 >= h3)
{
	...
	(
		explode a rec)100 200 300 400 350 500 600 500 400 1900 350 300 800 100 200 50

		v[0] for (int i = 0; i < 10 - 1; i++)
	{
		if (v[i] > v[i + 1])
			D--;
	}
...
{1, 2, 3, 4, 5} = 3 {6, 2, 3, 4, 5} = 4 {6, 7, 3, 4, 5} = 5

														  if (rocket going down)
															  parachute.explode;
